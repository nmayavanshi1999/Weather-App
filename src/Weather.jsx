...
export default function Weather() {
  ...
  const handleRefresh = () => fetchWeather(lastCity);
    ...
    <div style={{ ...s.page, background: theme }}>
      <div style={s.inner}>
        ...
        <div style={s.appTitle}>
          ...
          <p style={s.titleSub}>Real-time conditions anywhere in the world</p>
        </div>
        ...
        <div style={s.searchBox}>
          ...
          />
          <button style={{ ...s.searchBtn, opacity: loading ? 0.6 : 1 }} onClick={handleSearch} disabled={loading}>
            🔍 Search
          </button>
        </div>
        ...
        {weather && (
          <div style={s.card} aria-live="polite">
            ...
            <div style={s.cardHero}>
              <div style={s.heroTop}>
                <div>
                  <div style={s.cityName}>📍 {weather.name}</div>
                  <div style={s.cityCountry}>{weather.country}</div>
                </div>
                <div style={s.weatherIcon}>{weather.condition.icon}</div>
              </div>
              <div style={s.tempRow}>
                <span style={s.bigTemp}>{weather.temp}</span>
                <span style={s.tempUnit}>°C</span>
              </div>
              <div style={s.conditionText}>{weather.condition.text}</div>
            </div>
            ...
            <div style={s.statsGrid}>
              <StatItem icon="💧" label="Humidity"   value={weather.humidity} unit="%" />
              <StatItem icon="💨" label="Wind"        value={weather.wind}     unit="km/h" border />
              <StatItem icon="🌡️" label="Feels like" value={weather.feels}    unit="°C" border />
            </div>

            <div style={s.cardFooter}>
              <span style={s.footerText}>Updated {weather.updatedAt}</span>
...
function StatItem({ icon, label, value, unit, border }) {
  return (
    <div style={{ ...s.stat, borderLeft: border ? "1px solid rgba(255,255,255,0.2)" : "none" }}>
      <div style={s.statIcon}>{icon}</div>
      <div style={s.statLabel}>{label}</div>
      <div style={s.statVal}>
        {value}<span style={s.statUnit}> {unit}</span>
      </div>
    </div>
  );
}
...
function LoadingDots() {
  return (
    <span style={{ display: "inline-flex", gap: 4 }} aria-hidden="true">
      ...
      <style>{`@keyframes dp{0%,80%,100%{opacity:.2}40%{opacity:1}}`}</style>
    </span>
  );
}
...
const s = {
  ...
  appTitle: { textAlign: "center", marginBottom: "1.5rem" },
  h1: { fontSize: 28, fontWeight: 500, color: "#fff", margin: 0 },
  titleSub: { fontSize: 13, color: "rgba(255,255,255,0.75)", marginTop: 4 },
  ...
  cardHero: { padding: "1.75rem 1.5rem 1.25rem" },
  heroTop: { display: "flex", justifyContent: "space-between", alignItems: "flex-start" },
  cityName: { fontSize: 20, fontWeight: 500, color: "#fff" },
  cityCountry: { fontSize: 12, color: "rgba(255,255,255,0.7)", marginTop: 3 },
  weatherIcon: { fontSize: 52, lineHeight: 1 },
  tempRow: { display: "flex", alignItems: "flex-end", gap: 6, marginTop: "1rem" },
  bigTemp: { fontSize: 72, fontWeight: 500, color: "#fff", lineHeight: 1 },
  tempUnit: { fontSize: 28, color: "rgba(255,255,255,0.8)", marginBottom: 10 },
  conditionText: { fontSize: 14, color: "rgba(255,255,255,0.8)", marginTop: 6 },

  divider: { height: 1.5, background: "rgba(255,255,255,0.2)", margin: "0 1.5rem" },

  statsGrid: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr" },
  stat: { padding: "14px 10px", textAlign: "center" },
  statIcon: { fontSize: 20, marginBottom: 6 },
  statLabel: { fontSize: 11, color: "rgba(255,255,255,0.65)", marginBottom: 4 },
  statVal: { fontSize: 16, fontWeight: 500, color: "#fff" },
  statUnit: { fontSize: 11, color: "rgba(255,255,255,0.65)" },
